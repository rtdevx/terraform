...
resource "aws_lb_listener" "application_load_balancer_443" {
  ...
  default_action {
    ...
    fixed_response {
      ...
      message_body = "<html><body><center><h1>Fixed Static message for root content - SSL</h1></center><center><h2><a href=https://${aws_route53_record.cw.name}/app1/index.html>app1</a></h2></center></body></html>"
      status_code  = "200"
    }
  }
}
