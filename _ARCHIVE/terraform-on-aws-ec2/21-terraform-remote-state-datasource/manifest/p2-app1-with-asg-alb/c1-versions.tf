...
terraform {
  ...
  backend "s3" {
    bucket = "rk-backend"
    key    = "dev/p2-aws-vpc/terraform.tfstate"
    region = "eu-west-2"
    //dynamodb_table = "dev-p2-app1-with-asg-alb-lock" # NOTE: Uncomment to enable state locking with DynamoDB. Table must be created in `c1-dynamodb-lock.tf`.
    encrypt = true
  }
